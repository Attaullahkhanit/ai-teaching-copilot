...
function LessonPlan() {
  ...
  const configuration = new Configuration({
    apiKey: "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",
  });
  ...
  const callAIModel = async () => {
    ...
    let object = {
      ...option,
      prompt:location?.state?.isForEditer? text: `${location?.state?.p1} ${topicsInput} ${location?.state?.p2} ${studentAge} ${location?.state?.p3}`,
    }; 
